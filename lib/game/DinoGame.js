...
export default class DinoGame extends GameRunner {
  ...
  drawFPS() {
    this.paintText(
      'fps: ' + Math.round(this.frameRate), 0, 0,
      {
        font: 'PressStart2P-Regular',
        size: '12px',
        align: 'center',
  ...
  drawScore() {
    ...
    if (state.score.isBlinking) {
        state.score.blinkFrames++
        
        if (state.score.blinkFrames % state.settings.scoreBlinkRate === 0) {
            state.score.blinks++
            state.score.isBlinking = state.score.blinks < 6
            state.score.blinkFrames = 0
        }
    }
    ...
    const scoreText = state.score.value.toString()
    const scoreWidth = canvasCtx.measureText(scoreText).width
    const x = this.width - scoreWidth - 30

    canvasCtx.fillText(scoreText, x, 30)

    if (state.score.isBlinking) {
      canvasCtx.fillStyle = '#f44336'
      canvasCtx.fillRect(x, 30, scoreWidth, 4)
    }
    canvasCtx.restore()
  }
  /**
